...
class ThreadClient:
    ...
    def get_tool_outputs(self, run_id, thread_id):
        ...
        for action in run.required_action.submit_tool_outputs.tool_calls:
            ...
            outputs.append(
                {
                    "output": self.functions_map[function.name](
...
def main():
    ...
    if api_key == "":
        ...
        thread_client = ThreadClient(open_ai_client)
    ...
    if run_id != "":
        run = thread_client.get_run(run_id, thread_id)
        if run.status == "completed":
            st.success("The search is completed. The details are as follows")
            thread_client.get_messages(thread_id)
            ...
            with st.status("Processing action..."):
                thread_client.submit_tool_outputs(run_id, thread_id)
                time.sleep(3)
                st.rerun()
    ...
    if keyword:
        if thread_id == "":
            ...
            thread_client.send_message(thread_id, keyword)
